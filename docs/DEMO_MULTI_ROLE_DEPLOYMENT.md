...
仅在没有 TLS 的短期内部演示环境中，可以显式配置
`DGBOOK_DEMO_ALLOW_INSECURE_HTTP=1`，并为教师端、学生端配置解析到同一服务器的不同
HTTP 主机名。该开关不会改变默认的 HTTPS 要求，不得用于正式生产、长期公网服务或承载真实教学数据；
演示结束后应关闭，并迁移到受控域名和 HTTPS。

如果短期演示服务器只有一个公网 IPv4，且未备案域名会被接入商拦截，可以把教师端和投屏端配置为
普通 IPv4 地址，把学生端配置为该地址的 IPv4 映射 IPv6 字面地址。例如
`8.153.206.97` 对应 `http://[::ffff:899:ce61]`。浏览器会把二者视为不同 Cookie 主机，
而自动部署生成的 Nginx 配置会把两种地址转发到同一应用。该方案仍属于临时 HTTP 演示兼容，
不替代正式域名、ICP备案和 HTTPS。
